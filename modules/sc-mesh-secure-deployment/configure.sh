...
function server {
  ...
  if [ $execution_ctx = "br_hardened" ]; then
    ...
    if [ "$2" == "-t" ] ; then
        python3 $MESH_COM_ROOT$SERVER_SRC_PATH -c $MESH_COM_ROOT$KEY_PATH --test
    else
...
function client {
  ...
  if [ "$2" == "-t" ] ; then
    python3 $MESH_COM_ROOT$CLIENT_SRC_PATH -c $MESH_COM_ROOT$KEY_PATH -s http://$server_ip:5000 --test
  else
