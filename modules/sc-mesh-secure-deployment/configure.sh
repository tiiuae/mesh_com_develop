...
function server {
  ...
  if [ $execution_ctx = "br_hardened" ]; then
    ...
    if [ "$2" == "-t" ] ; then
        python3 $MESH_COM_ROOT$SERVER_SRC_PATH -c $MESH_COM_ROOT$KEY_PATH --test -m $mesh_mode
    else
        python3 $MESH_COM_ROOT$SERVER_SRC_PATH -c $MESH_COM_ROOT$KEY_PATH -m $mesh_mode
    fi
  else
    pushd .
    cd ../..
    ...
    # Install the server and provision the certificate generated by cryptolib
    if [ "$2" == "-t" ] ; then
        sudo python3 $MESH_COM_ROOT$SERVER_SRC_PATH -c $MESH_COM_ROOT$KEY_PATH --test
    else
